...
option_list = list(
  ...
  optparse::make_option(c("-n", "--no3_con"),   type="double",       default=0.5,       help="NO3- concentration, defalt: 0.5"),
...
# create a new folder:
new_folder = paste(otu_name,'_NO3_',NO3_con,'_iter_',iter_no,'/',sep = '')
...
# infile_my_model='OTU02_renamed/20210505_diet_sw_no_oxygen_STY_Merged_OTU02-draft__12_NO3/STY_Merged_OTU02.RDS' ## 20210505_diet_sw_no_oxygen_STY_Merged_OTU02-draft__12_NO3 is used in single species test and 3-taxa co-culture.
## 20210509_diet_sw_no_oxygen_STY_Merged_OTU02-draft__12_NH3_n_NO3 is used in the 8-taxa co-culture!
my_model <- readRDS(infile_my_model)

replicates <- Cl_no
cores <- Cl_no
cl <- makeCluster(cores, type="PSOCK")
clusterExport(cl, c("diet","my_model","getwd","new_folder","grid_no","death_r","Inocc_no","iter_no","replicates","cores","NO3_con"))
...
simlist <- parLapply(cl, 1:replicates, function(i){
  ...
  print("====================================================================")
  bacterium_OTU02 <- BacArena::Bac(my_model,lyse = F,deathrate=death_r,maxweight = 1, minweight=0.125,cellweight_mean = 0.5,cellweight_sd = 0.25, growtype='exponential',setAllExInf=TRUE,limit_growth = T)
  arena <- BacArena::Arena(n=grid_no,m=grid_no)
  inocc <- arena@n * arena@m * Inocc_no # 5% inocculum
  arena <- BacArena::addOrg(object = arena, specI = bacterium_OTU02, amount = inocc*1) # amount can be added by fraction.
  ...
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[44], smax = 1000, unit = "mM", add = F)    #[1] EX_cpd00067_e0	Proton (H+)
...
saveRDS(simlist, file = paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'_400grids_mineral_sw_rl.RDS',sep = ''))
# saveRDS(simulation, file = paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'_400grids_mineral_sw_rl.RDS',sep = ''))

# ############################ Download RDS file and run on MacOS ############################
getwd <- getwd()
getwd
simulation_loop <- readRDS(paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'_mineral_sw_rl.RDS',sep = ''))
