...
# create a new folder:
new_folder = paste(otu_name,'_NH3_',NH3_con,'_iter_',iter_no,'/',sep = '')
...
my_model <- readRDS(infile_my_model) #d__Archaea;p__Crenarchaeota;c__Nitrososphaeria;o__Nitrososphaerales;f__Nitrosopumilaceae;g__Cenarchaeum;s__

replicates <- Cl_no
cores <- Cl_no
cl <- makeCluster(cores, type="PSOCK")
clusterExport(cl, c("diet","my_model","getwd","new_folder","grid_no","death_r","Inocc_no","iter_no","replicates","cores","NH3_con"))
...
simlist <- parLapply(cl, 1:replicates, function(i){
  ...
  print("====================================================================")
  bacterium_OTU08 <- BacArena::Bac(my_model,lyse = F,deathrate=death_r,maxweight = 1, minweight=0.125,cellweight_mean = 0.5,cellweight_sd = 0.25, growtype='exponential',setAllExInf=TRUE,limit_growth = T)
  arena <- BacArena::Arena(n=grid_no,m=grid_no)
  inocc <- arena@n * arena@m * Inocc_no # 5% inocculum
  arena <- BacArena::addOrg(object = arena, specI = bacterium_OTU08, amount = inocc*1) # amount can be added by fraction.
...
getwd <- getwd()
getwd
simulation_loop <- readRDS(paste(getwd,'/',new_folder,'/BacArena_STY_',otu_name,'_400grids_mineral_sw_rl.RDS',sep = ''))
