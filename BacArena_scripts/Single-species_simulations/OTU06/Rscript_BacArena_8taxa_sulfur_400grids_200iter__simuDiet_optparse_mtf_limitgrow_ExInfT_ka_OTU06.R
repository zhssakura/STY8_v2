...
# create a new folder:
new_folder = paste(otu_name,'_NO2_',NO2_con,'_iter_',iter_no,'/',sep = '')
...
clusterExport(cl, c("diet","my_model","getwd","new_folder","grid_no","death_r","Inocc_no","iter_no","replicates","cores","NO2_con"))
...
simlist <- parLapply(cl, 1:replicates, function(i){
  ...
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[35], smax = 1000, unit = "mM", add = F) #[1] EX_cpd00013_e0	N as ammonia
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[36], smax = 0, unit = "mM", add = F) #[1] EX_cpd00528_e0	N as Dinitrogen N2
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[37], smax = 0, unit = "mM", add = F) #[1] EX_cpd00209_e0	N as nitrate
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[38], smax = NO2_con, unit = "mM", add = F) #[1] EX_cpd00075_e0	N as nitrite
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[40], smax = 1000, unit = "mM", add = F) #[1] EX_cpd00007_e0	Oxygen
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[42], smax = 0, unit = "mM", add = F) #[1] EX_cpd11632_e0	Photon hn
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[46], smax = 0, unit = "mM", add = F) #[1] EX_cpd00074_e0	S as Element sulfur S
  arena <- BacArena::addSubs(object = arena, mediac = diet$Exchange[48], smax = 0.001, unit = "mM", add = F) #[1] EX_cpd00239_e0	S as Sulfide (H2S)
