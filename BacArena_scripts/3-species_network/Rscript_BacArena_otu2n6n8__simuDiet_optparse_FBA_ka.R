...
############ Katana ############ 
diet <- readRDS('~/Gh_Sflabelliformis_8_MAGs/10_A_genome-scale_metabolic_network_for_S.flabelliformis_publication/Growth_media/3-species_network/mineral_sw_210514_OTU08_3_BacArena_realistic_simplified_9999_NH3_n_light.RDS')
getwd <- getwd()
getwd
...
# The photo-symbiont use nitrate -> NH3
STYtaxon_2='STY_Merged_OTU02.RDS'
...
print("============================ OTU2: The photo-symbiont use nitrate -> NH3 ========================================")
print("============================ OTU6: The NOB ========================================")
...
print("============================ OTU2:OTU6:OTU8(2.740:3.690:28.570); space = 400; [NH3] = see in the file name; nitrite=0; removeM=see in the file name; iter = 10 ========================================")
...
bacterium2 <- BacArena::Bac(model2,lyse = F,deathrate=death_r,maxweight = 1, minweight=0.125,cellweight_mean = 0.5,cellweight_sd = 0.25, growtype='exponential',setAllExInf=FALSE)

arena <- BacArena::Arena(n=grid_no,m=grid_no)
inocc <- arena@n * arena@m * 11.42857143/400 # 0.1% inocculum
arena <- BacArena::addOrg(object = arena, specI = bacterium6, amount = inocc*3.690) # amount can be added by fraction.
arena <- BacArena::addOrg(object = arena, specI = bacterium8, amount = inocc*28.570) # amount can be added by fraction.
arena <- BacArena::addOrg(object = arena, specI = bacterium2, amount = inocc*2.740) # amount can be added by fraction.
...
BacArena::chemotaxis(bacterium2, arena, 1, chemo='EX_cpd00209_e0', arena@occupyM)#nitrate
...
tp0_spe3_cell_no = length(which(tp0[["type"]] == '3')) # number of individuals of 3rd model (OTU2) at time point 0.
...
tp1_spe3_cell_no = length(which(tp1[["type"]] == '3')) # number of individuals of 3rd model (OTU2) at time point 1.
...
# OTU8:
tp0_spe2_ord<-which(tp0[["type"]] == '2') # Find the 2nd model (OTU8) in the orgdat at time point 0.
tp1_spe2_ord<-which(tp1[["type"]] == '2') # Find the 2nd model (OTU8) in the orgdat at time point 1.
# OTU2:
tp0_spe3_ord<-which(tp0[["type"]] == '3') # Find the 3rd model (OTU2) in the orgdat at time point 0.
tp1_spe3_ord<-which(tp1[["type"]] == '3') # Find the 3rd model (OTU2) in the orgdat at time point 1.
...
current_biomass_OTU2_tp0 =0 # assign a variable with integer of 0.
for (i in tp0_spe3_ord) {
  bb<-tp0[["biomass"]][i] # find the biomass of OTU6 at time point 0
  current_biomass_OTU2_tp0= current_biomass_OTU2_tp0+bb # calculate the total amount of bb
}

print(paste('↑↑↑Total biomass of simlist befor the 1st iteration (2)    = ', round(current_biomass_OTU6_tp0,5)+round(current_biomass_OTU8_tp0,5),', OTU6=',round(current_biomass_OTU6_tp0,5),', OTU8=',round(current_biomass_OTU8_tp0,5),', OTU2=',round(current_biomass_OTU2_tp0,5), sep = ''))# equal to arena 
print(paste('↑↑↑Total cells   of simlist befor the 1st iteration        = ', tp0_all_cell_no,', OTU6=',tp0_spe1_cell_no,', OTU8=',tp0_spe2_cell_no,', OTU2=',tp0_spe3_cell_no, sep = ''))# equal to arena 
...
for (i in tp1_spe2_ord) {
  bb<-tp1[["biomass"]][i] # find the biomass of OTU8 at time point 0
  current_biomass_OTU8_tp1= current_biomass_OTU8_tp1+bb # calculate the total amount of bb
}

current_biomass_OTU2_tp1 =0 # assign a variable with integer of 0.
for (i in tp1_spe3_ord) {
  bb<-tp1[["biomass"]][i] # find the biomass of OTU2 at time point 0
  current_biomass_OTU2_tp1= current_biomass_OTU2_tp1+bb # calculate the total amount of bb
}

print(paste('↑↑↑Total biomass of simlist after the 1st iteration (2)    = ', round(current_biomass_OTU6_tp1,5)+round(current_biomass_OTU8_tp1,5),', OTU6=',round(current_biomass_OTU6_tp1,5),', OTU8=',round(current_biomass_OTU8_tp1,5),', OTU2=',round(current_biomass_OTU2_tp1,5), sep = ''))
print(paste('↑↑↑Total cells   of simlist after the 1st iteration        = ', tp1_all_cell_no,', OTU6=',tp1_spe1_cell_no,', OTU8=',tp1_spe2_cell_no,', OTU2=',tp1_spe3_cell_no, sep = ''))# equal to arena 
print('                          ')
print('                          ')
...
for (i in 2:iter_no) {
  ...
  assign(name, round(bimass_plotGrothCurve[1,3]+bimass_plotGrothCurve[2,3]+bimass_plotGrothCurve[3,3],5))

  name <- paste("biomass_aft_tp", i, sep = "")
  assign(name, round(bimass_plotGrothCurve[4,3]+bimass_plotGrothCurve[5,3]+bimass_plotGrothCurve[6,3],5))
  ...
  assign(name, round(bimass_plotGrothCurve[5,3],5))

  name <- paste("biomass_OTU2_bf_tp", i, sep = "")
  assign(name, round(bimass_plotGrothCurve[3,3],5))

  name <- paste("biomass_OTU2_aft_tp", i, sep = "")
  assign(name, round(bimass_plotGrothCurve[6,3],5))
  ...
  #OTU2"  
  name <- paste("spe3_cell_no_bf_tp", i, sep = "")
  assign(name, length(which(tp0[["type"]] == '3')))

  name <- paste("spe3_cell_no_af_tp", i, sep = "")
  assign(name, length(which(tp1[["type"]] == '3')))

  print(paste('↑↑↑Total cells of simlist befor the ',i,'th iteration = ', length(tp0[["type"]]),'. OTU06:',length(which(tp0[["type"]] == '1')),', OTU08:',length(which(tp0[["type"]] == '2')),', OTU02:',length(which(tp0[["type"]] == '3')), sep = ''))
  print(paste('↑↑↑Total cells of simlist after the ',i,'th iteration = ', length(tp1[["type"]]),'. OTU06:',length(which(tp1[["type"]] == '1')),', OTU08:',length(which(tp1[["type"]] == '2')),', OTU02:',length(which(tp1[["type"]] == '3')), sep = ''))
  print('                          ')
  print('                          ')
...
print(paste('NH3_con','rm_rate','iter_no',
            ...
            'ratio of sp3/sp1 bf tp1','ratio of sp3/sp1 af tp1','ratio of sp3/sp1 bf tp2','ratio of sp3/sp1 af tp2','ratio of sp3/sp1 bf tp3','ratio of sp3/sp1 af tp3','ratio of sp3/sp1 bf tp4','ratio of sp3/sp1 af tp4','ratio of sp3/sp1 bf tp5','ratio of sp3/sp1 af tp5','ratio of sp3/sp1 bf tp6','ratio of sp3/sp1 af tp6','ratio of sp3/sp1 bf tp7','ratio of sp3/sp1 af tp7','ratio of sp3/sp1 bf tp8','ratio of sp3/sp1 af tp8','ratio of sp3/sp1 bf tp9','ratio of sp3/sp1 af tp9','ratio of sp3/sp1 bf tp10','ratio of sp3/sp1 af tp10',
            ...
            'ratio of sp3/sp1 bf tp1','ratio of sp3/sp1 af tp1','ratio of sp3/sp1 af tp41','ratio of sp3/sp1 af tp42','ratio of sp3/sp1 af tp43','ratio of sp3/sp1 af tp44','ratio of sp3/sp1 af tp45','ratio of sp3/sp1 af tp46','ratio of sp3/sp1 af tp47','ratio of sp3/sp1 af tp48','ratio of sp3/sp1 af tp49','ratio of sp3/sp1 af tp50',
            
            'tp0_spe1_cell_no,tp0_spe2_cell_no,tp0_spe3_cell_no','tp1_spe1_cell_no,tp1_spe2_cell_no,tp1_spe3_cell_no',
            'spe1_cell_no_af_tp2,spe2_cell_no_af_tp2,spe3_cell_no_af_tp2',
            'spe1_cell_no_af_tp3,spe2_cell_no_af_tp3,spe3_cell_no_af_tp3',
            'spe1_cell_no_af_tp4,spe2_cell_no_af_tp4,spe3_cell_no_af_tp4',
            'spe1_cell_no_af_tp5,spe2_cell_no_af_tp5,spe3_cell_no_af_tp5',
            'spe1_cell_no_af_tp6,spe2_cell_no_af_tp6,spe3_cell_no_af_tp6',
            'spe1_cell_no_af_tp7,spe2_cell_no_af_tp7,spe3_cell_no_af_tp7',
            'spe1_cell_no_af_tp8,spe2_cell_no_af_tp8,spe3_cell_no_af_tp8',
            'spe1_cell_no_af_tp9,spe2_cell_no_af_tp9,spe3_cell_no_af_tp9',
            'spe1_cell_no_af_tp10,spe2_cell_no_af_tp10,spe3_cell_no_af_tp10',
            'spe1_cell_no_af_tp42,spe2_cell_no_af_tp42,spe3_cell_no_af_tp42',
            'spe1_cell_no_af_tp43,spe2_cell_no_af_tp43,spe3_cell_no_af_tp43',
            'spe1_cell_no_af_tp44,spe2_cell_no_af_tp44,spe3_cell_no_af_tp44',
            'spe1_cell_no_af_tp45,spe2_cell_no_af_tp45,spe3_cell_no_af_tp45',
            'spe1_cell_no_af_tp46,spe2_cell_no_af_tp46,spe3_cell_no_af_tp46',
            'spe1_cell_no_af_tp47,spe2_cell_no_af_tp47,spe3_cell_no_af_tp47',
            'spe1_cell_no_af_tp48,spe2_cell_no_af_tp48,spe3_cell_no_af_tp48',
            'spe1_cell_no_af_tp49,spe2_cell_no_af_tp49,spe3_cell_no_af_tp49',
            'spe1_cell_no_af_tp50,spe2_cell_no_af_tp50,spe3_cell_no_af_tp50',
...
print(paste(NH3_con,rm_rate,iter_no,
            ...
            tp0_spe3_cell_no/tp0_spe1_cell_no,tp1_spe3_cell_no/tp1_spe1_cell_no,spe3_cell_no_bf_tp2/spe1_cell_no_bf_tp2,spe3_cell_no_af_tp2/spe1_cell_no_af_tp2,spe3_cell_no_bf_tp3/spe1_cell_no_bf_tp3,spe3_cell_no_af_tp3/spe1_cell_no_af_tp3,spe3_cell_no_bf_tp4/spe1_cell_no_bf_tp4,spe3_cell_no_af_tp4/spe1_cell_no_af_tp4,spe3_cell_no_bf_tp5/spe1_cell_no_bf_tp5,spe3_cell_no_af_tp5/spe1_cell_no_af_tp5,spe3_cell_no_bf_tp6/spe1_cell_no_bf_tp6,spe3_cell_no_af_tp6/spe1_cell_no_af_tp6,spe3_cell_no_bf_tp7/spe1_cell_no_bf_tp7,spe3_cell_no_af_tp7/spe1_cell_no_af_tp7,spe3_cell_no_bf_tp8/spe1_cell_no_bf_tp8,spe3_cell_no_af_tp8/spe1_cell_no_af_tp8,spe3_cell_no_bf_tp9/spe1_cell_no_bf_tp9,spe3_cell_no_af_tp9/spe1_cell_no_af_tp9,spe3_cell_no_bf_tp10/spe1_cell_no_bf_tp10,spe3_cell_no_af_tp10/spe1_cell_no_af_tp10,
            ...
            tp0_spe3_cell_no/tp0_spe1_cell_no,tp1_spe3_cell_no/tp1_spe1_cell_no,spe2_cell_no_af_tp41/spe1_cell_no_af_tp41,spe2_cell_no_af_tp42/spe1_cell_no_af_tp42,spe2_cell_no_af_tp43/spe1_cell_no_af_tp43,spe2_cell_no_af_tp44/spe1_cell_no_af_tp44,spe2_cell_no_af_tp45/spe1_cell_no_af_tp45,spe2_cell_no_af_tp46/spe1_cell_no_af_tp46,spe2_cell_no_af_tp47/spe1_cell_no_af_tp47,spe2_cell_no_af_tp48/spe1_cell_no_af_tp48,spe2_cell_no_af_tp49/spe1_cell_no_af_tp49,spe2_cell_no_af_tp50/spe1_cell_no_af_tp50,
